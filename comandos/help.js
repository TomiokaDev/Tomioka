...
module.exports = (client, message, args) => {      
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .addField("Comandos de reacción e interaccion", "`chocomilk` `owo` `uwu` `cookie` `clown` `question` `orientation` `microwave` `tabien` `tamal` `tamaomeno` `pan` `cyber` `trespingos` `punch` `kiss` `lasubeolabaja`")
