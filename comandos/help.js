...
module.exports = (client, message, args) => {      
  ...
  const embed = new Discord.MessageEmbed()
    ...
    .addField("Comandos de reacción", "`chocovaso` `owo` `cookie` `clown` `question` `genero` `microwave` `tabien` `tamal` `pan` `cyber`")
  ...
  const embednsfw = new Discord.MessageEmbed()
    .addField("Comandos útiles", "`report` `suggest` `anuncio`")
    .addField("Comandos de reacción", "`chocovaso` `owo` `cookie` `clown` `question` `genero` `microwave` `tabien` `tamal` `cyber`")
    .addField("Comandos de interaccion", "`punch` `kiss`")
    .addField("Comandos de información", "`owner` `info` `ping` `serverinfo` `donate`")
