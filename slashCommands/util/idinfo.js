...
if(isNaN(args[0])) return interaction.reply("Ingresa una ID")
if(args[0].length < 12) return interaction.reply("ID invalida")
let id = await message.client.users.fetch(args[0]);
if(!id) return interaction.reply("Usuario no encontrado. Ingresa una ID válida")
